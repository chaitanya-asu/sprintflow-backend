...
VALUES
  ...
  ('DotNet Sprint - C5', 'DotNet', 'C5',
   ...
   '04:00 PM', '05:00 PM',
   'Scheduled',
   '.NET training for C5 cohort. Session: 16:00-17:00.',
   NOW(), NOW()),
  ...
  ('Salesforce Sprint - C6', 'SalesForce', 'C6',
   ...
   '05:00 PM', '06:00 PM',
   'Scheduled',
   'Salesforce training for C6 cohort. Session: 17:00-18:00.',
   NOW(), NOW())
...
SELECT
  (SELECT id FROM sprints WHERE title = 'DotNet Sprint - C5' LIMIT 1),
  e.id, 'ENROLLED', NOW()
FROM employees e
WHERE e.technology = 'DotNet' AND e.cohort = 'C5'
ON DUPLICATE KEY UPDATE status = 'ENROLLED';

-- Enroll Salesforce students
INSERT INTO sprint_employees (sprint_id, employee_id, status, enrolled_at)
SELECT
  (SELECT id FROM sprints WHERE title = 'Salesforce Sprint - C6' LIMIT 1),
  e.id, 'ENROLLED', NOW()
FROM employees e
WHERE e.technology = 'SalesForce' AND e.cohort = 'C6'
ON DUPLICATE KEY UPDATE status = 'ENROLLED';

-- Add some attendance data for the new sprints
SET @dotnet_sprint = (SELECT id FROM sprints WHERE title = 'DotNet Sprint - C5' LIMIT 1);
SET @salesforce_sprint = (SELECT id FROM sprints WHERE title = 'Salesforce Sprint - C6' LIMIT 1);
