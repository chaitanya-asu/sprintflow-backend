-- ═══════════════════════════════════════════════════════════════
-- SprintFlow — Python C3 + Devops C4 Seed
-- ═══════════════════════════════════════════════════════════════

USE sprintflow_db;

-- ── Python C3 Employees (10) ─────────────────────────────────
...
-- ── Enroll Python C3 into Python Sprint ─────────────────────
INSERT INTO sprint_employees (sprint_id, employee_id, status, enrolled_at)
SELECT
  (SELECT id FROM sprints WHERE title = 'Python Sprint - C3' LIMIT 1),
  e.id, 'ENROLLED', NOW()
FROM employees e
WHERE e.technology = 'Python' AND e.cohort = 'C3'
ON DUPLICATE KEY UPDATE status = 'ENROLLED';

-- ── Enroll Devops C4 into Devops Sprint ─────────────────────
INSERT INTO sprint_employees (sprint_id, employee_id, status, enrolled_at)
SELECT
  (SELECT id FROM sprints WHERE title = 'Devops Sprint - C4' LIMIT 1),
  e.id, 'ENROLLED', NOW()
FROM employees e
WHERE e.technology = 'Devops' AND e.cohort = 'C4'
ON DUPLICATE KEY UPDATE status = 'ENROLLED';

-- ── Attendance for Python C3 (varied data) ───────────────────
SET @python_sprint = (SELECT id FROM sprints WHERE title = 'Python Sprint - C3' LIMIT 1);
...
JOIN (
  ...
  SELECT 'PC1002', '2026-03-27', 'DNA'    UNION ALL
  ...
  SELECT 'PC1004', '2026-03-30', 'DNA'    UNION ALL
  ...
  SELECT 'PC1007', '2026-03-26', 'DNA'    UNION ALL
  ...
  SELECT 'PC1009', '2026-03-31', 'DNA'    UNION ALL
...
-- ── Attendance for Devops C4 ─────────────────────────────────
SET @devops_sprint = (SELECT id FROM sprints WHERE title = 'Devops Sprint - C4' LIMIT 1);
...
JOIN (
  ...
  SELECT 'DC1002', '2026-03-26', 'DNA'    UNION ALL
  ...
  SELECT 'DC1005', '2026-03-27', 'DNA'    UNION ALL
